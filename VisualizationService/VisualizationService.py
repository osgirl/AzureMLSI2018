...
import VisualizationServiceConfig as cfg
...
@app.route('/persona/label/<string:img_id>', methods=["POST"])
def labelImage(img_id):
    print(img_id)
...
def servePersonaPage(persona_name):
    ...
    for image in image_list:
        page_html += "<div><img src='img/{0}':image/jpeg;base64;+encoded_string></img>{1}<form action='label/{0}' method='post'><button type='submit'>retrain</button></form></div>".format(image.assoc_image_id, "labeled" if image.label_assoc_flag else "predicted")
